class Member::MusicalPieceListsController < ApplicationController
  ...
  def create
    ...
    if @musical_piece_list.save
      redirect_back fallback_location: root_path, flash: {success: "マイリストに追加しました"}
    else
      redirect_back fallback_location: root_path, flash: {danger: "選択したリストにすでに追加済みか、または他の理由によりリストに追加できませんでした"}
    end
  end
  ...
  def destroy
    ...
    redirect_back fallback_location: root_path, flash: {success: "マイリストから削除しました"}
  end
